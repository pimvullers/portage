...
toolchain_src_install() {
	...
	if is_crosscompile; then
		...
		has noinfo ${FEATURES} \
			...
			|| prepinfo "${EPREFIX}${DATAPATH}"
		has noman ${FEATURES} \
			&& rm -r "${ED}/${DATAPATH}"/man \
			|| prepman "${EPREFIX}${DATAPATH}"
	fi
	...
	chown -R root:0 "${ED}"${LIBPATH} 2>/dev/null

	# Move pretty-printers to gdb datadir to shut ldconfig up
	local py gdbdir=${EPREFIX}/usr/share/gdb/auto-load${LIBPATH/\/lib\//\/$(get_libdir)\/}
...
gcc_movelibs() {
	...
	for multiarg in $($(XGCC) -print-multi-lib) ; do
		...
		for FROMDIR in \
			${EPREFIX}${LIBPATH}/${OS_MULTIDIR} \
			${EPREFIX}${LIBPATH}/../${MULTIDIR} \
			${EPREFIX}${PREFIX}/lib/${OS_MULTIDIR} \
			${EPREFIX}${PREFIX}/${CTARGET}/lib/${OS_MULTIDIR}
		do
		...
		FROMDIR="${EPREFIX}${PREFIX}/lib/${OS_MULTIDIR}"
...
do_gcc_config() {
	...
	current_gcc_config=$(env -i ROOT="${EROOT}" gcc-config -c ${CTARGET} 2>/dev/null)
...
fix_libtool_libdir_paths() {
	...
	sed -i \
		...
		$(find ./${EPREFIX}${PREFIX}/lib* -maxdepth 3 -name '*.la') \
		./${dir}/*.la
